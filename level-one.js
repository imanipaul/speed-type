...
const randomPosition = function() {
  ...
  position =
    Math.floor(Math.random() * game.clientHeight) + header.clientHeight;
...
const matchWord = function() {
  ...
  let currentWordsInner = [];
  currentWords.forEach(element => currentWordsInner.push(element.innerHTML));
  console.log("currentWordsInner", currentWordsInner);

  if (currentWordsInner.includes(textBox.value)) {
    currentWords.forEach(function(element) {
      if (textBox.value === element.innerHTML) {
        console.log("textbox value is", textBox.value);
        console.log("matched");
        element.classList.add("remove-word-transition");
